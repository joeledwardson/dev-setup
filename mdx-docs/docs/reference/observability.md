...
:::details Acronyms & jargon (click to expand)
...
- [The reframe — three problems, not one](#the-reframe--three-problems-not-one)
- [What real homelab setups actually run](#what-real-homelab-setups-actually-run)
- [The two core architectures](#the-two-core-architectures)
- [The base template (tiers)](#the-base-template-tiers)
- [The tools, with pictures](#the-tools-with-pictures)
- [What the metrics look like (CPU over time)](#what-the-metrics-look-like-cpu-over-time)
- [Dashboards — look vs be told](#dashboards--look-vs-be-told-parallel-to-alerts)
- [NixOS recipe](#nixos-recipe-shape)
- [Avoid](#avoid-for-this-scale--constraints)
- [TL;DR — what should I actually do?](#tldr--what-should-i-actually-do)
...
An agent per node auto-discovers thousands of metrics at **1-second** granularity into a local TSDB, with a zero-config dashboard on `:19999` and built-in health + ML-anomaly alerts. Gorgeous out of the box — but heavy on a constrained box (see [Avoid](#avoid-for-this-scale--constraints)).
...
![CPU % over 24h with an 85% alert threshold; a sustained spike crosses it and fires an alert, a brief blip is ignored](/assets/observability/cpu-over-time.svg)
...
|---|---|---|
| Instant, zero-config, deepest | **Netdata** | every metric at 1-second resolution the moment it's installed (but heavy — see [Avoid](#avoid-for-this-scale--constraints)) |
...
| **Netdata** | low | 1 per node | best instant dashboard (1 s, zero-config) | gorgeous and deep the moment it's installed | RAM growth on small boxes ([Avoid](#avoid-for-this-scale--constraints)); UI now proprietary |
