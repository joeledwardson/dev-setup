...
:::info Why `swhkd` not `sxhkd`, `wtype` not `xdotool`
...
:::info Why waystt needs an API key (and why `openai-whisper` didn't)
...
:::info Why not just one hardcoded check?
...
:::info What's confirmed vs inferred
...
:::info PAM is required for swaylock
...
:::details Key terms (plain English)
| Term | Plain English |
|---|---|
...
:::info IOPS vs read/write-time (throughput) monitoring — the key difference
...
:::details What `dd` is and how the IOPS test actually works (click to expand)
...
:::details Spotting a starved or throttled Pi — every method (click to expand)
...
:::info On a Pi, SATA vs NVMe doesn't matter — the USB port is the limit
...
:::info Prices/links captured 2026-06-14 via live browser (Brave). They move — treat as examples and check a tracker before buying. Full buy-vs-replace reasoning in [ADR-004](../appendix/adr/004-pi-box-storage-vs-replace).

:::

## 2026-06-18 Building the Pi on the SSD

:::info TL;DR — the whole journey, 4 steps
...
:::details Why this is a daemon, not the `whisper-ctranslate2 --live_transcribe` I'd been poking at
...
## bitwarden
Ahhh this old chesnut. claude found an interesting one...

```bash
bw get template item
```

gives what fields are required!

Then an example of actually *creating* an item (as my zsh functions are for editing and viewing not creating)
```bash
echo "$(jo -d. type=1 name='matrix-jollof' login.username='jollof' login.password='pls')" | bw encode | bw create item
```

> zsh swallows and errors if I use the default format of login[username] for nesting, so i use `.` as a delimiter


## matrix
Oh... very interesting one along the way. when fucking around with sudo based registration files. can use `sudoedit` which preserves some stuff? on the user side? otherwise copying to clipbaord doesnt work in vim.

So can replace this
```bash
sudo vim /var/lib/mautrix-telegram/telegram-registration.yaml
```

with this:
```bash
sudoedit /var/lib/mautrix-telegram/telegram-registration.yaml
```


## 2026-06-26 12:16 — fixed expand-upwards / expand-across window hotkeys

**Context:** the `SUPER+ALT+m` window menu (`configs/hypr/scripts/window-menu.sh`) has two actions — *expand-upwards* (make the active window full screen **height**) and *expand-across* (full **width**) — by shoving the windows above/below (resp. left/right) out of the way. Both did nothing.

**Root cause (reproduced live, not guessed):** the old script evacuated each blocking window to `special:magic` and *immediately moved it back to the same workspace*. In dwindle, removing a window and re-adding it reproduces the **identical** split — so the layout was byte-for-byte unchanged. A logical no-op, not a syntax bug. (The `hl.dsp.*` Lua dispatchers themselves work — this Hyprland runs the Lua-config plugin, which parses every `hyprctl dispatch` string as Lua, so `hyprctl keyword …` is dead and classic `movewindow r` syntax fails. Worth remembering.)

**The detour (worth recording as a lesson):** I first solved this the hard way — detect every window sharing the active window's column/row, then push them out one at a time with `movewindow <dir>`, recomputing after each move because dwindle restructures the tree (a single fixed-direction sweep tangled nested splits; a window pushed toward the monitor edge silently no-ops, so direction had to aim at whichever side already had a window). It worked (7/7 e2e) but was ~145 lines of geometry + jq + an iteration loop, and stayed illegible through three readability passes.

**Fix (what shipped):** there's a **native dwindle command that does the whole thing** — `layoutmsg movetoroot` promotes the active window to one half of the root split and lets Hyprland reflow everything else into the other half. It fills whichever axis the root split runs along; `togglesplit` flips that axis when it filled the wrong one. So:
- *expand-upwards* = `movetoroot`, then `togglesplit` if the window came out wider than tall;
- *expand-across* = `movetoroot`, then `togglesplit` if it came out taller than wide.

That's the entire script — ~15 lines, no geometry, no blocker detection, no loop. **Lesson: check the layout engine for a native primitive before reimplementing its tree logic in shell.** Tradeoff accepted: `movetoroot` is coarser than the surgical version — it reshuffles *all* other windows into the opposite half, not just the ones in the way — which is fine for "make this window big now".

**Result:** 6/6 e2e on the live compositor (upwards/across on simple + nested layouts, idempotent re-runs, single-window no-op). No `hyprctl reload` needed — the keybind execs the script (a symlink into this repo) fresh each press. Also confirmed: this Hyprland runs the Lua-config plugin, which parses every `hyprctl dispatch` string as Lua, so `hyprctl keyword …` is dead (use the `hl.dsp.*` forms) — worth remembering.
