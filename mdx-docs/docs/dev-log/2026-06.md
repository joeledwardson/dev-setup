...
## fix `expand-upwards` in hyprland

My `SUPER+ALT+m` menu's *expand-upwards* (full height) and *expand-across* (full width) both did nothing. The old script moved blocking windows to `special:magic` then back — but in dwindle that just recreates the same split. No-op.

I first fixed it the hard way (~145 lines pushing windows out one at a time), then found dwindle does it natively:

```bash
layoutmsg movetoroot   # window takes half the root split, rest reflows into the other half
togglesplit            # flip the axis if it filled the wrong one
```

~15 lines total.

> **Lesson:** check the layout engine for a native command before reimplementing its tree logic in shell.

Also: this Hyprland runs the Lua-config plugin, so `hyprctl dispatch` strings are parsed as Lua. `hyprctl keyword …` is dead — use the `hl.dsp.*` forms.

## visual replace nvim
Ahhh, I'm sure i've come across this before but here we are again...

When doing replace on a selection (when press `:` in nvim on a selection it auto changes it to `'<,'>:`). which means:
- the `'<` means first line of the charater selected? (from help)
- the `'>` the last line of the charater selected? 

ahhhh right kind of like `:0,3s/t/a` would change `t` to `a` from line 0 to 3

okok so that is JUST lines.

when in the context of patterns  `\%V` matches (ONLY) inside visual area - so this is for visual area vs lines. makes does now tbf
