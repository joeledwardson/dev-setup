-- nvim-min: the daily-driver config ported to Neovim 0.12 idioms.
--   * vim.pack instead of lazy.nvim (plugins load eagerly; lockfile is nvim-pack-lock.json)
--   * vim.lsp.enable + nvim-lspconfig definitions instead of Mason (servers come from flake.nix)
...
-- lua/plugins/. Run via the `nvim-min` wrapper (flake.nix); it never touches ~/.config/nvim.
