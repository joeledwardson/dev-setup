...
This local copy is derived from `coleam00/excalidraw-diagram-skill` and vendored into this repo for local maintenance.

Compatible with any coding agent that supports skills. For agents that read from `.claude/skills/` (like [Claude Code](https://docs.anthropic.com/en/docs/claude-code) and [OpenCode](https://github.com/nicepkg/OpenCode)), copy this directory into place and go.
...
Copy this vendored skill into your project's `.claude/skills/` directory:

```bash
cp -r configs/claude/skills/excalidraw-diagram .claude/skills/excalidraw-diagram
```
