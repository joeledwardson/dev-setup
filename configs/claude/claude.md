...
- New project: derive base via `uv run python -c "import zlib; print(9000 + zlib.crc32(b'<project>') % 900)"` (or the language equivalent), assign offsets, write numbers into the repo's `justfile` / `docker-compose.yml` / equivalents, and add a preflight step that fails loudly when a port is held by another process (printing pid+cmdline). Don't pick a fallback port — fix the conflict.
...
- **Every repo MUST have a `.registered-ports.toml` in its root** — this is how `port-dash` (at `:9999`) discovers all services. Flat TOML, one `service-name = port` per line. Example:

  ```toml
  # ~/my-project/.registered-ports.toml
  api          = 9123
  frontend     = 9124
  storybook    = 9125
  my-extra-svc = 9126
  ...
  The numbers here are the source of truth — must match `justfile` / `docker-compose.yml` / equivalents. The CRC32 rule is only how to *pick* on first allocation.
